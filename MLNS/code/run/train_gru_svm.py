...
from code.util.util import hinge_loss, EarlyStopping
...
print("Train Data.shape", data.shape, "Train Data label.shape", label.shape)
# split
...
def train_model(model, train_x, train_label, valid_x, valid_label, test_x, test_label,loss_fn):
    ...
    for epoch in range(model.config.num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            out = model(x_train)
            loss = loss_fn(model, out, label_train)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(valid_loader):
                ...
                loss = loss_fn(model, out, label_valid)
                # print(i + 1, 'loss: ',loss.item())
...
if __name__ == '__main__':
    ...
    model = model.cuda()
    train_model(model, train_x, train_label, valid_x, valid_label, test_x, test_label, hinge_loss)
